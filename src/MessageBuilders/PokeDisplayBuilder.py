...
def build_poke_display(bot, chat_id, trade_mode, page_num, poke_id):
    ...
    reply_keyboard = get_display_keyboard_editing(
        poke_id=pokemon.poke_id, page_num=page_num) if not trade_mode else get_display_keyboard_trading(
...
def poke_edit_name(bot, chat_id, pokemon_id):
    ...
    pokemon = DBAccessor.get_pokemon_by_id(int(pokemon_id))
...
def poke_edit_team(bot, chat_id, poke_id):
    ...
    if len(player.pokemon_team) >= 6:
        ...
        new_team_pokemon = DBAccessor.get_pokemon_by_id(int(poke_id))
        player.pokemon.remove(int(poke_id))
        player.pokemon_team.append(int(poke_id))
